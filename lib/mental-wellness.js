  ...
  async processUserMessage(userInput, context = {}) {
    try {
      ...
      const crisisSchema = z.object({
        urgencyLevel: z.enum(['CRISIS', 'HIGH', 'MODERATE', 'LOW']).describe('The crisis level classification.'),
      ...
      const result = await generateText({
        ...
        tools: {
          classification: {
            description: 'Classify the crisis level of the user message.',
            schema: crisisSchema
          }
        },
        toolChoice: 'required'
      });
      console.log('generateText result:', JSON.stringify(result, null, 2));
      const { toolResults } = result;
      
      const crisisAssessment = toolResults[0].result;
      const crisisLevel = crisisAssessment.urgencyLevel;
